...
class Battery < ActiveRecord::Base
  ...
  def self.getColumnas()
    [ 
     {:name => "Id",:key => "batteries.id",:related_attribute => "id", :width => 50},
     {:name => "Nro. Serial",:key => "batteries.serial_number",:related_attribute => "getSerialNumber()", :width => 120, 
       :selected => true},
     {:name => "En manos de",:key => "people.name",:related_attribute => "getOwner()", :width => 120},
     {:name => "Propietario CI",:key => "people.id_document",:related_attribute => "getOwnerIdDoc()", :width => 80, :datatype => "select", :options => "personas", :vista => "movements"},
     {:name => "Shipment",:key => "shipments.comment",:related_attribute => "getShipmentComment()", :width => 120},
     {:name => "Id Caja",:key => "batteries.box_serial_number",:related_attribute => "getBoxSerialNumber()", :width => 80},
     {:name => "Estado",:key => "statuses.description",:related_attribute => "getStatus()", :width => 80, :datatype => "combobox", :data => "buildSelectHash2(Status,id,\"getDescription()\",true,[])"},
     {:name => "Creado",:key => "batteries.created_at",:related_attribute => "getCreatedAt()", :width => 80, :datatype => "date"}
    ]
  end
  ...
  def getSubPartsOn
    cond = ["parts.on_device_serial = ? and parts.battery_id is not NULL", self.getSerialNumber]
    Part.find(:all, :conditions => cond)
  end

  def getLastMovementType
    inc = [:movement_details]
    cond = ["movement_details.battery_id = ?", self.id]
