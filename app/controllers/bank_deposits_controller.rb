class BankDepositsController < SearchController
  ...
  def new
    ...
    h = { "label" => "Deposito", "datatype" => "textfield" }.merge( bank_deposit ? {"value" => bank_deposit.getDeposit } : {} )
    @output["fields"].push(h)

    h = { "label" => "Monto", "datatype" => "numericfield" }.merge( bank_deposit ? {"value" => bank_deposit.getAmount } : {} )
    @output["fields"].push(h)

    fecha = Fecha.usDate(bank_deposit ? bank_deposit.getDepositedAt : Date.today.to_s)
    h = { "label" => "Fecha de Deposito", "datatype" => "date", :value => fecha  }
    @output["fields"].push(h)

    h = { "label" => "Banco", "datatype" => "textfield" }.merge( bank_deposit ? {"value" => bank_deposit.bank } : {} )
    @output["fields"].push(h)
